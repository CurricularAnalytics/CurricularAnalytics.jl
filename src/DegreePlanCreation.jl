...
function bin_filling(curric::Curriculum, additional_courses::Array{AbstractCourse}=Array{AbstractCourse,1}(); min_terms::Int=2, max_terms::Int=10, min_cpt::Int=3, max_cpt::Int=19)
    ...
    while length(UC) > 0
        if ((c = select_vertex(curric, term_courses, UC)) != nothing)
            ...
            for course in UC 
                for req in course.requisites[curric.requisite_clauses[course.id]]  
                    if req[1] == c.id  
                        if req[2] == strict_co  
    ...
    for target in UC
        ...
        if invariant1 == true
            ...
            for c in term_courses
                if c.id in collect(keys(target.requisites[curric.requisite_clauses[target.id]])) && target.requisites[curric.requisite_clauses[target.id]][c.id] == pre  # AND shortcircuits, otherwise 2nd expression would error
                    invariant2 = false
                    break  # try a new target
                end
            end
